/*
 ...
 Description : CRC 8 check
 ============================================================================
 */
#include "crc.h"

//	Check crc8 functions
int crc8(char * buf, int cnt)
{
    ...
    while (cnt--)
    {
    	crc ^= *(buf++);
	    for (i = 0; i < 8; i++)
	        crc = crc & 0x01 ? (crc >> 1) ^ CRC_XOR : crc >> 1;
	}
	return crc;
}
...
{
	uint16_t res = 0x9249;	//	Initial condition 1001 0010 0100 1001
	int i;
	for (i=0; i<cnt; i++)
			res += src[i];
	res^=0xAAAA;			//	XOR_OUT	1010 1010 1010 1010
	return res;
}
